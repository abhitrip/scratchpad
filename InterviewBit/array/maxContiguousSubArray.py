class Solution:
    ...
    def maxSubArray(self, A):
        sumSubArray = [x for x in A]
        numElements = len(A)
        for i in range(1,numElements):
            TentativeSum = sumSubArray[i-1]+sumSubArray[i]
            if TentativeSum > sumSubArray[i]:
                sumSubArray[i] = TentativeSum

        return max(sumSubArray)
...
if __name__=="__main__":
    ...
    sol = Solution()
    print 'maximum sum sumSubArray = %d' sol.maxSubArray(A)
